    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
        <div class="text-center">
            ...
            <div class="flex flex-col sm:flex-row gap-4 justify-center animate-fade-in-up animation-delay-600">
                ...
                @endauth
                <a href="https://wa.me/6281388556335" target="_blank"
...
<section id="buat-komplain" class="py-20 bg-white">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            {{-- Blocked: customer has an active complaint --}}
            <div class="rounded-xl border-2 border-orange-300 bg-orange-50 p-6">
                <div class="flex items-start">
                    <div class="flex-shrink-0">
                        <svg class="w-8 h-8 text-orange-500 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
                        </svg>
                    </div>
                    <div class="ml-4 flex-1">
                        <h3 class="text-lg font-bold text-orange-800 mb-2">Anda Masih Memiliki Komplain Aktif</h3>
                        <p class="text-orange-700 mb-4">
                            Anda tidak dapat membuat komplain baru selama komplain sebelumnya belum selesai. Tunggu hingga komplain Anda berstatus <strong>Selesai</strong> untuk dapat membuat komplain baru.
                        </p>
                        @if($activeComplaint)
                        <div class="bg-white border border-orange-200 rounded-lg p-4 mb-4">
                            <div class="grid grid-cols-2 gap-2 text-sm">
                                <div class="text-gray-500 font-medium">ID Komplain</div>
                                <div class="text-gray-800 font-semibold">#{{ $activeComplaint->id }}</div>
                                <div class="text-gray-500 font-medium">Kategori</div>
                                <div class="text-gray-800">{{ $activeComplaint->category->name ?? '-' }}</div>
                                <div class="text-gray-500 font-medium">Status</div>
                                <div>
                                    @php($statusColor = match($activeComplaint->status){ 'baru' => 'bg-red-100 text-red-700', 'diproses' => 'bg-yellow-100 text-yellow-700', default => 'bg-gray-100 text-gray-700' })
                                    <span class="px-2 py-1 text-xs font-semibold rounded {{ $statusColor }} capitalize">{{ $activeComplaint->status }}</span>
                                </div>
                                <div class="text-gray-500 font-medium">Tanggal Dibuat</div>
                                <div class="text-gray-800">{{ $activeComplaint->created_at->format('d/m/Y H:i') }}</div>
                            </div>
                        </div>
                        @endif
                        <a href="{{ route('complaints.show', $activeComplaint) }}" class="inline-flex items-center px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white rounded-lg text-sm font-medium transition-colors duration-200">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                            </svg>
                            Lihat Komplain Aktif
                        </a>
                    </div>
                </div>
            </div>
            ...
            <div class="text-center py-12 bg-blue-50 rounded-lg border border-blue-200">
                ...
                <div class="mt-6 pt-6 border-t border-blue-200">
                    ...
                    <div class="flex justify-center">
                        <a href="https://wa.me/6281388556335" target="_blank" class="text-green-600 hover:text-green-700 font-medium flex items-center justify-center">
                            <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.885 3.488"/>
                            </svg>
...
@auth
@if(Auth::user()->role === 'customer' && $customerStats['total'] > 0)
<section class="bg-white border-t border-gray-100 py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">

            {{-- Total Komplain --}}
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 group hover:border-blue-200 transition-colors">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-500 mb-1">Total Komplain</p>
                        <p class="text-2xl font-semibold text-gray-900">{{ $customerStats['total'] }}</p>
            ...
            {{-- Diselesaikan --}}
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 group hover:border-green-200 transition-colors">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-500 mb-1">Diselesaikan</p>
                        <p class="text-2xl font-semibold text-green-600">{{ $customerStats['done'] }}</p>
                        <p class="text-xs text-gray-400 mt-1">Aktif: {{ $customerStats['active'] }}</p>
                    </div>
            ...
            {{-- Skor Penyelesaian --}}
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 group hover:border-gray-300 transition-colors">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-500 mb-1">Skor Penyelesaian</p>
                        <p class="text-2xl font-semibold {{ $customerStats['rate'] >= 80 ? 'text-green-600' : ($customerStats['rate'] >= 50 ? 'text-yellow-600' : 'text-red-500') }}">
                            {{ $customerStats['rate'] }}%
                        </p>
                        <p class="text-xs text-gray-400 mt-1">Dari total komplain</p>
                    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <div class="flex items-center justify-between mb-6">
                ...
                <a href="#buat-komplain" 
                   class="{{ $hasActiveComplaint ? 'bg-gray-300 text-gray-500 cursor-not-allowed px-4 py-2 rounded-lg' : 'bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700' }}"
                   {{ $hasActiveComplaint ? 'title="Selesaikan komplain aktif Anda terlebih dahulu"' : '' }}>
                    {{ $hasActiveComplaint ? '🔒 Ada Komplain Aktif' : 'Buat Komplain Baru' }}
                </a>
            </div>
